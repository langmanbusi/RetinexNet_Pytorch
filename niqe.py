...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    img2 = np.array(Image.fromarray(img).resize((int(h/2), int(w/2))))
...
def niqe(inputImgData):
    ...
    # TODO: memoize
    params = scipy.io.loadmat(join(module_path, 'mat', 'niqe_image_params.mat'))
...
if __name__ == "__main__":
    ...
    input_list = glob('./data1/test/low/*.png')
    gt_list = glob('./data1/test/high/*.png')
    result_list = glob('./results/test/low/*.jpg')
    N = len(input_list)

    dict_all = {'input_list': input_list,
                'gt_list': gt_list,
    ...
    for key in dict_all.keys():
        pic_list = dict_all[key]
        with tqdm(pic_list) as pic_names:
            for pic_name in pic_names:
                pic_names.set_description(f"NIQE of {key}")
                pic = np.array(Image.open(pic_name).convert('LA'))[:, :, 0]
                score = niqe(pic)
                niqe_all[key] += [score]
                pic_names.set_postfix({'score': '{:.3f}'.format(score)})

    input_score = sum(niqe_all['input_list']) / N
    gt_score = sum(niqe_all['gt_list']) / N
    result_score = sum(niqe_all['result_list']) / N

    print('input NIQE: {:.3f}'.format(input_score),
          'gt NIQE: {:.3f}'.format(gt_score),
          'result NIQE: {:.3f}'.format(result_score))
